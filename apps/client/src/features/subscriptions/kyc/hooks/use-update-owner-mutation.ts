...
export function useUpdateKycOwnerMutation(subscriptionId: string) {
	const { t } = useTranslation("features.subscriptions.kyc.hooks.use-mutation");
	const queryClient = useQueryClient();

	return useMutation(
		api.subscriptions.updateKycOwner.mutationOptions({
			scope: { id: `subscription:${subscriptionId}:kyc-owner:update` },
			onSuccess: () =>
