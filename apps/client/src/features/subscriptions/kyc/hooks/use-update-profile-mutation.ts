...
export function useUpdateKycProfileMutation(subscriptionId: string) {
	const { t } = useTranslation("features.subscriptions.kyc.hooks.use-mutation");
	const queryClient = useQueryClient();

	return useMutation(
		api.subscriptions.updateKycProfile.mutationOptions({
			scope: { id: `subscription:${subscriptionId}:kyc-profile` },
			onSuccess: () =>
