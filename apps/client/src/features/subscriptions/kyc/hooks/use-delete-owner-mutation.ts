...
export function useDeleteKycOwnerMutation(subscriptionId: string) {
	const { t } = useTranslation("features.subscriptions.kyc.hooks.use-mutation");
	const queryClient = useQueryClient();

	return useMutation(
		api.subscriptions.deleteKycOwner.mutationOptions({
			scope: { id: `subscription:${subscriptionId}:kyc-owner:delete` },
			onSuccess: () =>
