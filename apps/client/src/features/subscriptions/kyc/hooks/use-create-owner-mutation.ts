...
export function useCreateKycOwnerMutation(subscriptionId: string) {
	const { t } = useTranslation("features.subscriptions.kyc.hooks.use-mutation");
	const queryClient = useQueryClient();

	return useMutation(
		api.subscriptions.createKycOwner.mutationOptions({
			scope: { id: `subscription:${subscriptionId}:kyc-owner:create` },
			onSuccess: () =>
