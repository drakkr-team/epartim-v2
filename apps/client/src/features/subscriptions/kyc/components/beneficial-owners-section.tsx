...
type BeneficialOwnersSectionProps = {
	createKycOwner: ReturnType<typeof useKycForm>["createKycOwner"];
	deleteKycOwner: ReturnType<typeof useKycForm>["deleteKycOwner"];
	form: ReturnType<typeof useKycForm>["form"];
	updateKycOwner: ReturnType<typeof useKycForm>["updateKycOwner"];
};

export function BeneficialOwnersSection(props: BeneficialOwnersSectionProps) {
	const { createKycOwner, deleteKycOwner, form, updateKycOwner } = props;
								...
								<div className="grid gap-4">
									{owners.map((owner, index) => (
										<BeneficialOwnerCard
											...
											onUpdate={(changes) => updateKycOwner(owner.id, changes)}
											owner={owner}
										/>
									))}
								</div>
