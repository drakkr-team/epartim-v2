...
export function useUpdateLegalAgentMutation(subscriptionId: string) {
	const { t } = useTranslation(
		"features.subscriptions.representatives_and_authorizations.hooks.use-update-mutation",
	);

	return useMutation(
		api.subscriptions.updateLegalAgent.mutationOptions({
			scope: { id: `subscription:${subscriptionId}:legal-agent` },
			onError: (error) => {
