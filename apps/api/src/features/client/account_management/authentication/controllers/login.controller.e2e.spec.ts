...
test.group(
	"Features / Account Management / Authentication / Controllers / Login Controller",
	() => {
		test("it should successfully login the user with correct credentials", async ({ client }) => {
			...
			const response = await client.visit("client.account_management.authentication.login").json({
				uid: email,
				password,
			});
		...
		test("it should response with E_GUEST_ONLY code if the user is already authenticated", async ({
			...
			const response = await client
				.visit("client.account_management.authentication.login")
				.loginAs(user)
				.json({
					uid: user.email,
					password: "password",
