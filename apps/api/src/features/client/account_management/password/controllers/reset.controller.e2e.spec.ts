...
import SendPasswordChangedNotification from "#features/client/account_management/password/jobs/send_password_changed_notification.job";
...
test.group("Features / Account Management / Password / Controllers / Reset Controller", (group) => {
	...
	test("it should reset the password with a valid token and push a password changed notification job", async ({
		...
		const response = await client.visit("client.account_management.password.reset").json({
			token,
			newPassword,
		});
	...
	test("it should respond with E_INVALID_TOKEN when already used token is provided", async ({
		...
		await client.visit("client.account_management.password.reset").json({
			token,
	...
	test("it should respond with E_INVALID_TOKEN when expired token is provided", async ({
		...
		const user = await UserFactory.create();
		const key = `user:${user.id}:reset-password`;
		const token = await otpService.generate({
			key,
			type: "alphanumeric",
			length: 32,
	...
	test("it should respond with E_GUEST_ONLY code if the user is already authenticated", async ({
		...
		const response = await client
			.visit("client.account_management.password.reset")
			.loginAs(user)
			.json({
				token: "valid-token",
				newPassword: "newpassword",
