...
import SendResetPasswordInstruction from "#features/client/account_management/password/jobs/send_reset_password_instruction.job";

test.group(
	"Features / Account Management / Password / Controllers / Forgot Controller",
	(group) => {
		group.each.teardown(() => {
		...
		test("it should respond with no content and push a reset password job for an existing email", async ({
			...
			const response = await client.visit("client.account_management.password.forgot").json({
				email: user.email,
			});
		...
		test("it should respond with E_GUEST_ONLY code if the user is already authenticated", async ({
			...
			const response = await client
				.visit("client.account_management.password.forgot")
				.loginAs(user)
				.json({
					email: user.email,
				});
