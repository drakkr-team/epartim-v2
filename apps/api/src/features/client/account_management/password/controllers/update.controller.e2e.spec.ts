...
test.group(
	"Features / Account Management / Password / Controllers / Update Controller",
	(group) => {
		group.each.teardown(() => {
		...
		test("it should respond with no content, persist the new password, and push a password changed notification job", async ({
			...
			const response = await client
				.visit("client.account_management.password.update")
				.loginAs(user)
				.json({
					currentPassword,
					newPassword,
		...
		test("it should respond with E_UNAUTHENTICATED when the user is not authenticated", async ({
			...
			const response = await client.visit("client.account_management.password.update").json({
