...
export default class ListCompaniesService {
	...
	#orderByQuery(query: ModelQueryBuilderContract<typeof Company>, orderBy: string) {
		const options = [
	...
	#searchQuery(query: ModelQueryBuilderContract<typeof Company>, search: string) {
		return query.whereILike("companies.name", `%${search}%`);
	}
}
