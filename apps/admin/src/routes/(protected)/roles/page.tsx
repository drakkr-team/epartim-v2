...
import { useRolesTable } from "#/features/roles/hooks/use-table";
...
export const Route = createFileRoute("/(protected)/roles/")({
	...
	loader: async ({ context, deps }) => {
		await context.queryClient.query({
			...api.roles.list.queryOptions({ query: deps }),
			staleTime: "static",
		});
	},
	component: Page,
...
function Page() {
	...
	const { t } = useTranslation("routes.(protected).roles");
	const searchParams = Route.useSearch();

	const { data: roles } = useSuspenseQuery(
		api.roles.list.queryOptions({ query: searchParams }, { placeholderData: keepPreviousData }),
	);
			...
			<DataTable table={table}>
				<div className="flex flex-wrap items-center justify-between gap-4">
					...
					<div className="flex items-center gap-2">
						{roles.meta.canCreate && (
							<Button variant="primary" nativeButton={false} render={<Link to="/roles/new" />}>
								<PlusIcon />
								{t("actions.new")}
							</Button>
						)}
