...
import { useUsersTable } from "#/features/users/hooks/use-table";
...
function Page() {
	...
	const { t } = useTranslation("routes.(protected).users");
	const searchParams = Route.useSearch();

	const { data: users } = useSuspenseQuery(
		api.users.list.queryOptions({ query: searchParams }, { placeholderData: keepPreviousData }),
	);
			...
			<DataTable table={table}>
				<div className="flex items-center justify-between gap-4">
					...
					<div className="flex items-center gap-2">
						{users.meta.canCreate && (
							<Button variant="primary" render={<Link to="/users/new" />}>
								<PlusIcon />
								{t("actions.new")}
							</Button>
						)}
