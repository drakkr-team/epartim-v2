...
export function useCreateRoleMutation() {
	const { t } = useTranslation("features.roles.hooks.use-create-mutation");

	const queryClient = useQueryClient();
	const navigate = useNavigate();

	return useMutation(
		api.roles.create.mutationOptions({
			onSuccess: async (role) => {
				await queryClient.invalidateQueries({ queryKey: api.roles.pathKey() });
				toast.success(t("success.title"), {
					description: t("success.description", { name: role.name }),
				});
				navigate({
					to: "/roles/$roleId",
