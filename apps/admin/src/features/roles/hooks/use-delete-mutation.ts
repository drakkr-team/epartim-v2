...
export function useDeleteRoleMutation() {
	const { t } = useTranslation("features.roles.hooks.use-delete-mutation");

	const queryClient = useQueryClient();

	return useMutation(
		api.roles.delete.mutationOptions({
			onSuccess: async () => {
				await queryClient.invalidateQueries({ queryKey: api.roles.list.pathKey() });
