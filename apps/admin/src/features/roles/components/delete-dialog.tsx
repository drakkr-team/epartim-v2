...
import { useDeleteRoleMutation } from "#/features/roles/hooks/use-delete-mutation";

type DeleteRoleDialogProps = AlertDialogProps & {
	role: Role;
	afterDelete?: () => void;
};

export function DeleteRoleDialog(props: DeleteRoleDialogProps) {
	const { role, afterDelete, ...rest } = props;

	const { t } = useTranslation("features.roles.components.delete-dialog");

	const { mutateAsync: deleteRole, isPending } = useDeleteRoleMutation();

	const handleDelete = async () => {
		await deleteRole({ params: { roleId: role.id } });
		afterDelete?.();
	};
