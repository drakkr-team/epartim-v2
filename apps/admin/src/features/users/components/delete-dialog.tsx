...
import { useDeleteUserMutation } from "#/features/users/hooks/use-delete-mutation";

type DeleteUserDialogProps = AlertDialogProps & {
	user: User;
	afterDelete?: () => void;
};

export function DeleteUserDialog(props: DeleteUserDialogProps) {
	const { user, afterDelete, ...rest } = props;

	const { t } = useTranslation("features.users.components.delete-dialog");

	const { mutateAsync: deleteUser, isPending } = useDeleteUserMutation();

	const handleDelete = async () => {
		await deleteUser({ params: { userId: user.id } });
		afterDelete?.();
	};
