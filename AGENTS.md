...
- The versioned Yaak workspace lives in `.yaak/`. Add every new or changed API route to Yaak, keeping its method, path, request body, and authentication requirements aligned with the implementation. Never store credentials or tokens in a shared Yaak environment.
